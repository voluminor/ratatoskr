...
const (
	...
	defaultCoreStopTimeout = 10 * time.Second
)
...
type ConfigObj struct {
	...
	CoreStopTimeout string   `json:"core_stop_timeout"`
...
func loadConfig(path string) (ConfigObj, time.Duration, error) {
	...
	timeout := defaultCoreStopTimeout
	if cfg.CoreStopTimeout != "" {
		timeout, err = time.ParseDuration(cfg.CoreStopTimeout)
		if err != nil {
