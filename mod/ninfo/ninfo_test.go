...
func TestAddSigil_invalidName(t *testing.T) {
	obj := newTestObj()
	errs := obj.AddSigil(newMockSigil("AB")) // too short + uppercase
	if len(errs) != 1 {
		t.Fatalf("expected 1 error, got %d", len(errs))
	}
...
func TestImportSigils_append(t *testing.T) {
	...
	src := &SigilsObj{sigils: map[string]sigils.Interface{
		"new-one": newMockSigil("new-one"),
	}}
