...
- **Узкие контракты модулей.** Корневой пакет предоставляет `core.Interface`; `socks`, `peermgr` и `forward`
  принимают меньшие локальные интерфейсы только с нужными им операциями. `core.Obj` удовлетворяет им структурно,
  а отдельные модули могут работать с пользовательскими транспортами без импорта полного core.
...
управление узлом (multicast, admin, retry, диагностика) доступно через `Core()`. SOCKS5-прокси,
...
flowchart LR
  ...
  Core --> NI["ninfo.New()"]
  NI --> PM{Peers задан?}
  PM -->|Да| Start["peermgr.New() запускает manager"]
...
- Если `cfg.Sigils != nil` — NodeInfo атомарно собирается из сигилов; любая ошибка сборки или парсера возвращает
  `ErrInvalidSigils`, узел не создаётся
...
- Если создание ломается после запуска core, rollback ограничен тем же `CloseTimeout`, что и `Close`
...
Предупреждение: `Core().EnableAdmin` — тонкий проброс к небезопасному upstream admin-сокету. В нём нет аутентификации,
дедлайнов и ограничения размера запроса; регистрация обработчиков может гоняться с запросами; остановка не закрывает
принятые keepalive-соединения; ошибки bind и очистки Unix-сокета могут вызвать `os.Exit(1)`. Используйте только
защищённый Unix-сокет или loopback для доверенного операционного доступа. Полный список ограничений приведён в
...
Если `Close` запускается одновременно с setter, обновление может успеть попасть в SOCKS-лимитер до возврата
`ErrClosed`; долговременного эффекта это не имеет, поскольку proxy уже завершает работу.
...
Подробнее о выборе, ограниченных batch-циклах, восстановлении после полного outage и валидации пиров —
в [mod/peermgr/README.md](mod/peermgr/README.md).
...
(64-символьный hex, `<hex>.pk.ygg`, `[ipv6]:port` или голый IPv6). IPv6 может быть как
адресом узла, так и хостом внутри маршрутизируемой Yggdrasil-подсети `/64`. Возвращает распарсенные
...
if err != nil {
    log.Printf("частичный NodeInfo: %v", err)
}
...
`Ask` и `AskAddr` могут вернуть ненулевой частичный результат вместе с ошибкой, в том числе если `Close` выиграл гонку
с уже завершившимся запросом. Если частичные метаданные полезны, вызывающий код должен проверить результат до
обработки ошибки. Сетевые методы в такой же гонке намеренно возвращают `nil` вместе с `ErrClosed`: выдача живого
соединения или listener после начала завершения привела бы к утечке ресурса и нарушила контракт закрытого узла.

Внутренне `ninfo` создаётся всегда при `New()`. Пользовательские невстроенные сигилы из `ConfigObj.Sigils`
добавляются к неизменяемым parser-прототипам из `ConfigObj.NodeInfo.Sigils`. Подробнее — в
...
2. Любая ошибка сигила прерывает `New`; частично собранный NodeInfo не публикуется
3. Результат записывается в `Config.NodeInfo` перед запуском core
4. Пользовательские невстроенные сигилы становятся неизменяемыми parser-прототипами для `Ask`/`AskAddr`
...
| `Peers`        | `*peermgr.ConfigObj` | `nil`        | Менеджер пиров; `Node` заменяется этим core. Не `nil` + `Config.Peers` → ошибка  |
| `NodeInfo`     | `*ninfo.ConfigObj`   | `nil`        | Тайминги `Ask`/`AskAddr` и парсеры ответов; `Source` заменяется core этого узла  |
| `Sigils`       | `[]sigils.Interface` | `nil`        | Атомарная сборка NodeInfo; пользовательские сигилы также парсят удалённые ответы |
...
| `MaxAssociateTargetsPerPrincipal`    | int                          | `0`          | Общий лимит целей на пользователя или исходный IP. `<=0` — без лимита; per-server лимит остаётся                                   |
| `MaxAssociateQueuedPacketsPerTarget` | int                          | `0`          | Лимит пакетов в очереди одной UDP-цели. `0` — 64, `<0` — без лимита                                                                |
| `MaxAssociateQueuedBytesPerTarget`   | int                          | `0`          | Лимит payload-байт в очереди одной UDP-цели. `0` — 64 КиБ, `<0` — без лимита                                                       |
...
| `CloseTimedOut` | `bool`              | Истёк бюджет остановки корневого объекта |
...
| `ActiveAssociateTargets`   | `int`    | Установленные UDP ASSOCIATE-цели               |
| `PendingAssociateTargets`  | `int64`  | Создаваемые цели                               |
| `RejectedAssociateTargets` | `uint64` | Цели, отклонённые admission-лимитами           |
| `DroppedAssociatePackets`  | `uint64` | Пакеты, отброшенные полной per-target очередью |
...
| `ErrClosed`                | Ошибочный метод корневого фасада вызван после `Close`          |
| `ErrCloseTimedOut`         | `Close` или rollback в `New` превысил `CloseTimeout`           |
| `ErrInvalidCloseTimeout`   | `CloseTimeout` отрицательный                                   |
| `ErrInvalidNodeInfo`       | Пользовательский NodeInfo нельзя безопасно клонировать         |
| `ErrInvalidSigils`         | Некорректна сборка локальных сигилов или конфигурация парсеров |

Сетевые методы, операции с пирами, SOCKS и NodeInfo на корневом фасаде после остановки единообразно возвращают ошибку,
совпадающую с `ErrClosed`. Низкоуровневые вызовы через `Core()` сохраняют контракт модуля, включая
`core.ErrNotAvailable`.
`New` сохраняет sentinel-ошибки валидации модулей, включая ошибки `peermgr`.

---

## Потокобезопасность

Публичные методы безопасны для конкурентного использования, кроме явно небезопасных upstream admin-хуков ниже.
...
| `Core().EnableMulticast` / `EnableAdmin` | Lifecycle компонентов защищён; регистрация upstream-handlers может гоняться с запросами |
...
| `Ask` / `AskAddr`                        | Потокобезопасны; ctx отменяет ожидание shared flight                                    |
...
`Core().EnableAdmin` делегирует работу upstream admin-сокету через `mod/core/admin`. Интерфейс намеренно небезопасен:
регистрация обработчиков может гоняться с запросами, принятые keepalive-соединения переживают `DisableAdmin`, а ошибки
bind и очистки Unix-сокета могут завершить процесс через `os.Exit(1)`. Подробнее — в
[предупреждении пакета admin](mod/core/admin/README.md).
...
| [`mod/peermgr`](mod/peermgr/README.md)   | Менеджер пиров: bounded-пробинг, outage recovery, выбор пиров         |
...
| [`mod/ninfo`](mod/ninfo/README.md)       | Запрос NodeInfo и неизменяемая конфигурация пользовательских парсеров |
