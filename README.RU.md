...
- **Probe** (`mod/probe`) — исследование топологии сети и трассировка маршрутов
...
// Создание модуля probe — требует raw yggdrasil-go/core, не core.Interface
...
```go
// Установить до вызова probe.New()
probe.CacheTTL = 30 * time.Second // быстрое истечение кеша
probe.MaxPeersPerNode = 1000 // ниже лимит для безопасности
probe.PollInterval = 100 * time.Millisecond // более частый опрос
probe.LookupRetryEvery = 500 * time.Millisecond
probe.HopsWaitTimeout = 5 * time.Second // дольше ждать hops
```
...
| `probe.Tree`          | `ErrMaxDepthRequired`, отмена контекста                                                   |
| `probe.Path`          | `ErrInvalidKeyLength`, `ErrKeyNotInTree`, `ErrTreeEmpty`, `ErrNoRoot`                     |
| `probe.Hops`          | `ErrInvalidKeyLength`, `ErrNoActivePath`                                                  |
| `probe.Trace`         | `ErrInvalidKeyLength`, `ErrLookupTimedOut` (может содержать частичный результат)          |
